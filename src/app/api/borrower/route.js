...
import {
  ...
  getAllLoans,
} from "@/lib/repositories/loans.repo";
...
export async function GET(request) {
  try {
    ...
    if (!borrowerID)
      return NextResponse.json({ error: 'חסר מספר ת"ז' }, { status: 400 });

    const borrower = getBorrowerById(borrowerID);
    if (!borrower)
      return NextResponse.json(
        { error: "תלמיד לא נמצא במערכת" },
        { status: 404 },
      );

    // השתמש ב-ID האמיתי מה-DB (לא מה שהוזן)
    const realID = borrower.borrowerID;

    // השאלות פעילות עם פרטי ספר
    const activeLoans = getActiveLoansByBorrower(realID).map((loan) => {
      const book = getBookById(loan.bookID);
      const now = new Date();
      const due = loan.dueDate
        ? new Date(loan.dueDate.split("/").reverse().join("-"))
        : null;
    ...
    const history = getAllLoans()
      .filter((l) => l.borrowerID === realID && l.ReturnAtDate?.trim())
      .slice(-20)
      .map((loan) => ({ ...loan, book: getBookById(loan.bookID) }));

    // רשימת המתנה
    const waitlist = getBorrowerWaitlist(realID).map((w) => ({
      ...w,
